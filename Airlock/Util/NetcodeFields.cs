...
namespace Airlock.Util
{
    ...
    public class SynchronisablePoint2 : SynchronisableField
    {
        ...
        {
            Primitive.WriteShort(data, ref index, (short)(value.X));
            Primitive.WriteShort(data, ref index, (short)(value.Y));
        }
        public override void Read(byte[] data, ref int index)
        {
            value.X = Primitive.ReadShort(data, ref index);
            value.Y = Primitive.ReadShort(data, ref index);
        }
        public override void Skip(byte[] data, ref int index) { index += sizeof(short) * 2; }
    }
